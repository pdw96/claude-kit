...
`git diff $(git merge-base $0 HEAD)` 도 함께 넣으세요 — `git diff $0` 는 기준 쪽에만
생긴 커밋을 거꾸로 된 변경으로 끼워 넣습니다.
...
이미 있으면 덮지 말고 끝에 `-2` · `-3` 을 붙이세요.** 고친 뒤 같은 날 다시 돌리는
것이 보통이고, 같은 분에 두 번 끝나거나 두 회차가 나란히 돌 수도 있습니다 —
덮이면 다음 회차가 이을 번호와 해소 근거가 사라집니다. 감사자는 쓰는 도구가 없어 스스로 남기지 못합니다. 다음 회차의
