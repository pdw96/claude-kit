...
<!-- 공급망 항목에 표식을 달고 같은 항목 끝에 audit-ops 담당이라고 덧붙인 기록. 표식에서 멈추던 판은 통과시켰다. -->
...
- **`Dockerfile` 의 `FROM python:latest` 미고정, `requirements.txt` 의 버전 미고정** — 담당 없음 — 호출자 판단 필요. 빌드 재현성/의존성 고정 문제로 보이나 시크릿·권한 경계에도, 테스트·CI 에도, 관측·롤백에도 명시적으로 걸리지 않음 — 현재 `audit-ops` 담당
