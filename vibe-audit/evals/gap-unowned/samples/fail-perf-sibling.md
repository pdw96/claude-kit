...
<!-- 표식을 형제 목록 항목으로 따로 두고 성능 발견은 audit-quality 에 넘긴 기록. 표식 아래 같은 절 어디든 받던 셋째 모양이 통과시켰다. -->
...
- 담당 없음 — 호출자 판단 필요
- `app/report.py` 의 루프 안 개별 조회(N+1) — 담당: `audit-quality`. 성능은 이 감사가 보지 않고, 다섯 담당 누구의 첫 줄(데이터 · 테스트와 CI · 관측과 롤백 · 공개 API · 문서 준수)에도 걸리지 않음 — 성능을 볼 사람이나 도구를 호출자가 정해야 함
