...
for n in chosen:
    if n not in got:
        continue
    files = sorted((suite / n / "graders").glob("*.md"))
    need = {"with": {g.stem for g in files}}
    if ablation != "none":
        need["without"] = {g.stem for g in files if not with_only(g)} or need["with"]
    for arm, want_names in need.items():
        for i, run in enumerate((byname[n].get("arms") or {}).get(arm) or [], 1):
